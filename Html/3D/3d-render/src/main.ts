...
const controlZ = document.querySelector('#z');
const controlScale = document.querySelector('#scale');

const loop = () => {
    ctx!.clearRect(0, 0, width, height);

    // @ts-ignore
    model.rotation[0] = parseInt(controlX.value) / 15
    // @ts-ignore
    model.rotation[1] = parseInt(controlY.value) / 15
    // @ts-ignore
    model.rotation[2] = parseInt(controlZ.value) / 15
    // @ts-ignore
    model.scale = parseInt(controlScale.value)
